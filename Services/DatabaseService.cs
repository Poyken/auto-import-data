    ...
    public class DatabaseService
    {
        ...
        public async Task<bool> IsFileImportedAsync(string filePath) 
        {
            try
            {
                ...
                // Câu lệnh SQL: Đếm (COUNT) xem có dòng nào trong bảng ExcelImportHistory_test có đường dẫn file khớp không.
                // Chỉ tính các file đã nạp thành công (Status = 'Success').
                string sql = "SELECT COUNT(*) FROM ExcelImportHistory_test WHERE FilePath = @path AND Status = 'Success'";
            ...
            using (SqlConnection conn = new SqlConnection(_config.ConnectionString)) 
            {
                ...
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        {
                            // Chỉ định tên bảng đích trong Database là "SortingDataImportExcel_test".
                            bulkCopy.DestinationTableName = "SortingDataImportExcel_test"; 
                        ...
                        // Sau khi nạp xong dữ liệu máy đo, ta ghi thêm 1 dòng vào bảng "ExcelImportHistory_test".
                        // Để đánh dấu: "Tui đã nạp thành công file này nhé!".
                        long fileSize = new FileInfo(filePath).Length;
                        string sql = "INSERT INTO ExcelImportHistory_test (FilePath, FileSize, ImportedAt, RowsInserted, Status) " +
